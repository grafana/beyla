...
int uprobe_server_handleStream(struct pt_regs *ctx) {
    ...
    if (bpf_map_update_elem(&ongoing_grpc_server_requests, &goroutine_key, &invocation, BPF_ANY)) {
...
int uprobe_netFdReadGRPC(struct pt_regs *ctx) {
    ...
    void *tr = bpf_map_lookup_elem(&ongoing_grpc_operate_headers, &goroutine_key);
...
int uprobe_http2Server_operateHeaders(struct pt_regs *ctx) {
    ...
    bpf_map_update_elem(&ongoing_grpc_operate_headers, &goroutine_key, &tr, BPF_ANY);
...
int uprobe_server_handler_transport_handle_streams(struct pt_regs *ctx) {
    ...
    if (parent_go.addr) {
        bpf_dbg_printk("found parent goroutine for transport handler [%llx]", parent_go.addr);
...
int uprobe_server_handleStream_return(struct pt_regs *ctx) {
    ...
    grpc_srv_func_invocation_t *invocation =
        bpf_map_lookup_elem(&ongoing_grpc_server_requests, &goroutine_key);
    if (invocation == NULL) {
        bpf_dbg_printk("can't read grpc invocation metadata");
        goto done;
    }

    u16 *status_ptr = bpf_map_lookup_elem(&ongoing_grpc_request_status, &goroutine_key);
    u16 status = 0;
    if (status_ptr != NULL) {
    ...
    bpf_map_delete_elem(&ongoing_grpc_server_requests, &goroutine_key);
    bpf_map_delete_elem(&ongoing_grpc_request_status, &goroutine_key);
...
int uprobe_transport_writeStatus(struct pt_regs *ctx) {
    ...
    if (status_ptr != NULL) {
        ...
        if (s_ptr != NULL) {
            ...
            bpf_map_update_elem(&ongoing_grpc_request_status, &goroutine_key, &status, BPF_ANY);
        }
    }
...
static __always_inline void clientConnStart(void *goroutine_addr, void *cc_ptr, void *ctx_ptr, void *method_ptr, void *method_len) {
    ...
    // Write event
    if (bpf_map_update_elem(&ongoing_grpc_client_requests, &goroutine_key, &invocation, BPF_ANY)) {
...
static __always_inline int grpc_connect_done(struct pt_regs *ctx, void *err) {
    ...
    grpc_client_func_invocation_t *invocation =
        bpf_map_lookup_elem(&ongoing_grpc_client_requests, &goroutine_key);
    ...
    bpf_map_delete_elem(&ongoing_grpc_client_requests, &goroutine_key);
    return 0;
}
...
int uprobe_transport_http2Client_NewStream(struct pt_regs *ctx) {
    ...
    if (t_ptr) {
        ...
        grpc_client_func_invocation_t *invocation = bpf_map_lookup_elem(&ongoing_grpc_client_requests, &goroutine_key);
    ...
    if (invocation) {
        ...
        if (w_ptr) {
            ...
            if (offset < MAX_W_PTR_OFFSET) {
                ...
                bpf_map_update_elem(&grpc_framer_invocation_map, &goroutine_key, &f_info, BPF_ANY);
            } else {
...
int uprobe_grpcFramerWriteHeaders_returns(struct pt_regs *ctx) {
    ...
    grpc_framer_func_invocation_t *f_info = bpf_map_lookup_elem(&grpc_framer_invocation_map, &goroutine_key);
    ...
    bpf_map_delete_elem(&grpc_framer_invocation_map, &goroutine_key);
    return 0;
}
#else
SEC("uprobe/grpcFramerWriteHeaders_returns")
