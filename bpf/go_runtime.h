...
int uprobe_proc_newproc1(struct pt_regs *ctx) {
    ...
    if (bpf_map_update_elem(&newproc1, &goroutine_key, &invocation, BPF_ANY)) {
...
int uprobe_proc_newproc1_ret(struct pt_regs *ctx) {
    ...
    void *creator_goroutine = GOROUTINE_PTR(ctx);
    bpf_dbg_printk("creator_goroutine_addr %lx", creator_goroutine);
    goroutine_key_t creator_goroutine_key = {
        .pid = bpf_get_current_pid_tgid(),
        .addr = (__u64)creator_goroutine,
    };    
    ...
    new_func_invocation_t *invocation =
        bpf_map_lookup_elem(&newproc1, &creator_goroutine_key);
    if (invocation == NULL) {
    ...
    bpf_map_delete_elem(&newproc1, &creator_goroutine_key);
