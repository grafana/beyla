...
static __always_inline void *extract_traceparent_from_req_headers(void *headers_ptr_ptr) {
    ...
    res = bpf_probe_read(&log_2_bucket_count,
                         sizeof(log_2_bucket_count),
                         headers_ptr + OFFSET_OF_GO_RUNTIME_HMAP_FIELD_B);
    if (res < 0) {
        return NULL;
    }
    u64 bucket_count = 1 << log_2_bucket_count;
    void *header_buckets;
    res = bpf_probe_read(&header_buckets,
                         sizeof(header_buckets),
                         headers_ptr + OFFSET_OF_GO_RUNTIME_HMAP_FIELD_BUCKETS);
    if (res < 0) {
        return NULL;
    }
    u32 map_id = 0;
    struct map_bucket *map_value =
        (struct map_bucket *)bpf_map_lookup_elem(&golang_mapbucket_storage_map, &map_id);
    if (!map_value) {
        return NULL;
    }
    ...
    for (u64 j = 0; j < MAX_BUCKETS; j++) {
        ...
        res = bpf_probe_read(
            map_value, sizeof(struct map_bucket), header_buckets + (j * sizeof(struct map_bucket)));
        if (res < 0) {
            continue;
        }
        for (u64 i = 0; i < 8; i++) {
            ...
            if (bpf_memicmp(current_header_key,
                            "traceparent",
                            W3C_KEY_LENGTH)) // grpc headers don't get normalized
            {
                continue;
            }
