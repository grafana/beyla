...
#define bpf_for_each(type, cur, args...)                                                                                                                       \
    for (/* initialize and define destructor */                                                                                                                \
         struct bpf_iter_##type ___it __attribute__((                                                                                                          \
             ...
             cleanup(                                                                                                                                          \
                 bpf_iter_##type##_destroy))), /* ___p pointer is just to call bpf_iter_##type##_new() *once* to init ___it */                                 \
         *___p __attribute__((unused)) =                                                                                                                       \
             (bpf_iter_##type##_new(&___it, ##args),                                                                                                           \
              /* this is a workaround for Clang bug: it currently doesn't emit BTF */ /* for bpf_iter_##type##_destroy() when used from cleanup() attribute */ \
...
#define bpf_for(i, start, end)                                                                                                                            \
    for (/* initialize and define destructor */                                                                                                           \
         struct bpf_iter_num ___it __attribute__((                                                                                                        \
             ...
             cleanup(                                                                                                                                     \
                 bpf_iter_num_destroy))), /* ___p pointer is necessary to call bpf_iter_num_new() *once* to init ___it */                                 \
         *___p __attribute__((unused)) =                                                                                                                  \
             (bpf_iter_num_new(&___it, (start), (end)),                                                                                                   \
              /* this is a workaround for Clang bug: it currently doesn't emit BTF */ /* for bpf_iter_num_destroy() when used from cleanup() attribute */ \
