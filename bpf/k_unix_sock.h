...
int BPF_KPROBE(be_k_unix_recvmsg, struct socket *sock, struct msghdr *msg, size_t size, int flags) {
...
int BPF_KRETPROBE(be_k_unix_recvmsg_ret, size_t copied) {
    u64 id = bpf_get_current_pid_tgid();
...
int BPF_KPROBE(be_k_unix_sendmsg, struct socket *sock, struct msghdr *msg, size_t size) {
    u64 id = bpf_get_current_pid_tgid();
...
int BPF_KRETPROBE(be_k_unix_sendmsg_ret, int sent_len) {
    u64 id = bpf_get_current_pid_tgid();
