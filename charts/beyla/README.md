...
| injector.enabled | bool | `false` | Whether to use Beyla in conjuction with the k8s-injecton-controller to aid in auto-instrumentation with the OpenTelemetry SDKs. This option grants the DaemonSet permission to create and update the injector state ConfigMaps in Beyla's own namespace (via a namespaced Role). Enable this alongside the `injector` settings in the Beyla configuration. Requires `rbac.create` to be true. |
