...
The `prometheus.scrape` section configures Prometheus scrape to collect metrics from Beyla. The `honor_labels = true` setting preserves Beyla's per-process instance identifiers. The `prometheus.remote_write` section sends the metrics to Grafana Cloud Prometheus.
