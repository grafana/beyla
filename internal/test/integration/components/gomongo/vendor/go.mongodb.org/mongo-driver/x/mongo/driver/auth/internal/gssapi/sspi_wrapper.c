...
int sspi_init() {
  // Load the secur32.dll library using its exact path. Passing the exact DLL
  // path rather than allowing LoadLibrary to search in different locations
  // removes the possibility of DLL preloading attacks. We use
  // GetSystemDirectoryA and LoadLibraryA rather than the
  // GetSystemDirectory/LoadLibrary aliases to ensure the ANSI versions are used
  // so we don't have to account for variations in char sizes if UNICODE is
  // enabled.

  // Passing a 0 size will return the required buffer length to hold the path,
  ...
  // Allocate a buffer to hold the system directory + "\secur32.dll" (length 12,
  ...
  INIT_SECURITY_INTERFACE init_security_interface =
      (INIT_SECURITY_INTERFACE)GetProcAddress(sspi_secur32_dll,
                                              SECURITY_ENTRYPOINT);
...
int sspi_client_init(sspi_client_state *client, char *username,
                     char *password) {
  TimeStamp timestamp;
  ...
  if (username) {
    if (password) {
      ...
      client->status = sspi_functions->AcquireCredentialsHandle(
          NULL, SSPI_PACKAGE_NAME, SECPKG_CRED_OUTBOUND, NULL, &auth_identity,
          NULL, NULL, &client->cred, &timestamp);
    } else {
      client->status = sspi_functions->AcquireCredentialsHandle(
          username, SSPI_PACKAGE_NAME, SECPKG_CRED_OUTBOUND, NULL, NULL, NULL,
          NULL, &client->cred, &timestamp);
    }
  } else {
    client->status = sspi_functions->AcquireCredentialsHandle(
        NULL, SSPI_PACKAGE_NAME, SECPKG_CRED_OUTBOUND, NULL, NULL, NULL, NULL,
        &client->cred, &timestamp);
  }
...
int sspi_client_username(sspi_client_state *client, char **username) {
  SecPkgCredentials_Names names;
  client->status = sspi_functions->QueryCredentialsAttributes(
      &client->cred, SECPKG_CRED_ATTR_NAMES, &names);
...
int sspi_client_negotiate(sspi_client_state *client, char *spn, PVOID input,
                          ULONG input_length, PVOID *output,
                          ULONG *output_length) {
  SecBufferDesc inbuf;
  SecBuffer in_bufs[1];
  ...
  client->status = sspi_functions->InitializeSecurityContext(
      &client->cred, client->has_ctx > 0 ? &client->ctx : NULL, (LPSTR)spn,
      ISC_REQ_ALLOCATE_MEMORY | ISC_REQ_MUTUAL_AUTH, 0, SECURITY_NETWORK_DREP,
      client->has_ctx > 0 ? &inbuf : NULL, 0, &client->ctx, &outbuf,
      &context_attr, NULL);
...
int sspi_client_wrap_msg(sspi_client_state *client, PVOID input,
                         ULONG input_length, PVOID *output,
                         ULONG *output_length) {
  SecPkgContext_Sizes sizes;

  client->status = sspi_functions->QueryContextAttributes(
      &client->ctx, SECPKG_ATTR_SIZES, &sizes);
  if (client->status != SEC_E_OK) {
    return SSPI_ERROR;
  }

  char *msg =
      malloc((sizes.cbSecurityTrailer + input_length + sizes.cbBlockSize) *
             sizeof(char));
  ...
  client->status = sspi_functions->EncryptMessage(
      &client->ctx, SECQOP_WRAP_NO_ENCRYPT, &wrap_buf_desc, 0);
  if (client->status != SEC_E_OK) {
    free(msg);
    return SSPI_ERROR;
  }
  ...
  *output_length =
      wrap_bufs[0].cbBuffer + wrap_bufs[1].cbBuffer + wrap_bufs[2].cbBuffer;
  *output = malloc(*output_length);

  memcpy(*output, wrap_bufs[0].pvBuffer, wrap_bufs[0].cbBuffer);
  memcpy(*output + wrap_bufs[0].cbBuffer, wrap_bufs[1].pvBuffer,
         wrap_bufs[1].cbBuffer);
  memcpy(*output + wrap_bufs[0].cbBuffer + wrap_bufs[1].cbBuffer,
         wrap_bufs[2].pvBuffer, wrap_bufs[2].cbBuffer);
...
int sspi_client_destroy(sspi_client_state *client) {
  if (client->has_ctx > 0) {
