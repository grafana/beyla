...
OM_uint32 gssapi_canonicalize_name(OM_uint32 *minor_status, char *input_name,
  ...
  major_status =
      gss_import_name(minor_status, &buffer, input_name_type, &imported_name);
  if (GSS_ERROR(major_status)) {
    return major_status;
  }

  major_status = gss_canonicalize_name(minor_status, imported_name,
                                       (gss_OID)gss_mech_krb5, output_name);
...
int gssapi_error_desc(OM_uint32 maj_stat, OM_uint32 min_stat, char **desc) {
  ...
  do {
    local_maj_stat = gss_display_status(&local_min_stat, stat, stat_type,
                                        GSS_C_NO_OID, &msg_ctx, &desc_buffer);
...
int gssapi_client_init(gssapi_client_state *client, char *spn, char *username,
                       char *password) {
  client->cred = GSS_C_NO_CREDENTIAL;
  client->ctx = GSS_C_NO_CONTEXT;

  client->maj_stat = gssapi_canonicalize_name(
      &client->min_stat, spn, GSS_C_NT_HOSTBASED_SERVICE, &client->spn);
  ...
  if (username) {
    gss_name_t name;
    client->maj_stat = gssapi_canonicalize_name(&client->min_stat, username,
    ...
    if (password) {
      ...
      client->maj_stat = gss_acquire_cred_with_password(
          &client->min_stat, name, &password_buffer, GSS_C_INDEFINITE,
          GSS_C_NO_OID_SET, GSS_C_INITIATE, &client->cred, NULL, NULL);
    } else {
      client->maj_stat = gss_acquire_cred(
          &client->min_stat, name, GSS_C_INDEFINITE, GSS_C_NO_OID_SET,
          GSS_C_INITIATE, &client->cred, NULL, NULL);
    }
...
int gssapi_client_username(gssapi_client_state *client, char **username) {
  OM_uint32 ignored;
  gss_name_t name = GSS_C_NO_NAME;

  client->maj_stat = gss_inquire_context(&client->min_stat, client->ctx, &name,
  ...
  client->maj_stat =
      gss_display_name(&client->min_stat, name, &name_buffer, NULL);
  if (GSS_ERROR(client->maj_stat)) {
    gss_release_name(&ignored, &name);
    return GSSAPI_ERROR;
  }
...
int gssapi_client_negotiate(gssapi_client_state *client, void *input,
  ...
  client->maj_stat = gss_init_sec_context(
      &client->min_stat, client->cred, &client->ctx, client->spn, GSS_C_NO_OID,
...
int gssapi_client_wrap_msg(gssapi_client_state *client, void *input,
                           size_t input_length, void **output,
                           size_t *output_length) {
  gss_buffer_desc input_buffer = GSS_C_EMPTY_BUFFER;
  gss_buffer_desc output_buffer = GSS_C_EMPTY_BUFFER;
  ...
  client->maj_stat =
      gss_wrap(&client->min_stat, client->ctx, 0, GSS_C_QOP_DEFAULT,
               &input_buffer, NULL, &output_buffer);
...
int gssapi_client_destroy(gssapi_client_state *client) {
  OM_uint32 ignored;
