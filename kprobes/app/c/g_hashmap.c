...
// g_hashmap_set_allocator allows for configuring a custom allocator for
// all hashmap library operations. This function, if needed, should be called
// only once at startup and a prior to calling g_hashmap_new().
...
// g_hashmap_new_with_allocator returns a new hash map using a custom allocator.
// See g_hashmap_new for more information information
...
// g_hashmap_new returns a new hash map. 
...
// comes with two helper functions `g_hashmap_sip()` and `g_hashmap_murmur()`.
// Param `compare` is a function that compares items in the tree. See the 
// qsort stdlib function for an example of how this function works.
// The hashmap must be freed with g_hashmap_free(). 
...
struct hashmap *g_hashmap_new(size_t elsize, size_t cap, 
                            ...
                            void *udata)
{
    return g_hashmap_new_with_allocator(
...
// g_hashmap_clear quickly clears the map. 
// Every item is called with the element-freeing function given in g_hashmap_new,
...
static bool resize(struct hashmap *map, size_t new_cap) {
    struct hashmap *map2 = g_hashmap_new_with_allocator(map->malloc, map->realloc, map->free,
...
// g_hashmap_set inserts or replaces an item in the hash map. If an item is
// replaced then it is returned otherwise NULL is returned. This operation
// may allocate memory. If the system is unable to allocate additional
// memory then NULL is returned and g_hashmap_oom() returns true.
...
// g_hashmap_get returns the item based on the provided key. If the item is not
// found then NULL is returned.
void *g_hashmap_get(struct hashmap *map, const void *key) {
    if (!key) {
        panic("key is null");
    }
...
// g_hashmap_probe returns the item in the bucket at position or NULL if an item
...
// g_hashmap_delete removes an item from the hash map and returns it. If the
// item is not found then NULL is returned.
void *g_hashmap_delete(struct hashmap *map, void *key) {
    if (!key) {
        panic("key is null");
    }
    map->oom = false;
...
// g_hashmap_count returns the number of items in the hash map.
size_t g_hashmap_count(struct hashmap *map) {
    return map->count;
}

// g_hashmap_free frees the hash map
// Every item is called with the element-freeing function given in g_hashmap_new,
// if present, to free any data referenced in the elements of the hashmap.
...
// g_hashmap_oom returns true if the last g_hashmap_set() call failed due to the 
// system being out of memory.
bool g_hashmap_oom(struct hashmap *map) {
    return map->oom;
}

// g_hashmap_scan iterates over all items in the hash map
...
// g_hashmap_iter iterates one key at a time yielding a reference to an
// entry at each iteration. Useful to write simple loops and avoid writing
// dedicated callbacks and udata structures, as in g_hashmap_scan.
//
...
// modified.
//
// Note that if g_hashmap_delete() is called on the hashmap being iterated,
...
// g_hashmap_sip returns a hash value for `data` using SipHash-2-4.
...
// g_hashmap_murmur returns a hash value for `data` using Murmur3_86_128.
