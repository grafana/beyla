...
func TestWatcher_Poll(t *testing.T) {
	...
	acc := pollAccounter{
		...
		loadBPFWatcher: func(*config.Config, chan<- watcher.Event) error {
			return nil
		},
	}
...
func TestPortsFetchRequired(t *testing.T) {
	...
	cfg, err := config.LoadConfig(userConfig)
	...
	acc := pollAccounter{
		...
		loadBPFWatcher: func(cfg *config.Config, events chan<- watcher.Event) error {
