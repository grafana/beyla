...
type bpf_tp_debugAsyncIdToFdKey struct {
	PidTgid uint64
	AsyncId uint64
}
...
type bpf_tp_debugConnectionInfoPartT struct {
	Addr [16]uint8
	Pid  uint32
	Port uint16
	Type uint8
...
type bpf_tp_debugFdInfoT struct {
	Pid  bpf_tp_debugPidKeyT
	Fd   int32
	Type uint32
}

type bpf_tp_debugFdKey struct {
	PidTgid uint64
	Fd      int32
	Pad     [4]uint8
}
...
type bpf_tp_debugNodeClientRequestKey struct {
	PidTgid         uint64
