...
func NewNetlinkManager() TCManager {
	return &netlinkManager {
		...
		programs: []*netlinkProg{},
		mutex: sync.Mutex{},
		log: slog.With("component", "netlink_manager"),
	}
}

func (tc *netlinkManager) Init(ctx context.Context) {
...
func (tc *netlinkManager) registerFilter(linkIndex int, fd int, handle uint32, parent uint32, name string) (*netlink.BpfFilter, error) {
	attrs := netlink.FilterAttrs{
		LinkIndex: linkIndex,
		Parent:    parent,
		Handle:    handle,
	...
	if err := netlink.FilterDel(filter); err == nil {
		tc.log.Warn("filter already existed. Deleted it", "filter", name, "iface", linkIndex)
	}
