...
// '\0'                                                         // End of message
func parsePostgresError(buf []uint8, length uint32) *SQLError {
	var (
		sqlErr        SQLError
		codeParsed    bool
		messageParsed bool
		offset        uint32 = POSTGRES_HDR_SIZE
	)

	if length < POSTGRES_ERR_MIN_LEN {
...
Loop:
	for offset < length {
		...
		switch errorCode {
		case 'C':
			sqlErr.SQLState = string(buf[offset : offset+5])
			offset += 6 // 5 bytes for SQLState + 1 byte for null terminator
			codeParsed = true
		case 'M':
			sqlErr.Message = trimNulls(string(buf[offset:]))
			offset += uint32(len(sqlErr.Message))
			messageParsed = true
		case 0:
			// End of message
			break Loop
		default:
			// Skip unknown fields while increasing the offset
			for offset < length {
				if buf[offset] == 0 {
					break
				}
	...
	if codeParsed && messageParsed {
		sqlErr.DB = DBTypePostgres
		return &sqlErr
	}
	return nil
}
...
var pgSQLStates = map[string]string{
...
func validatePostgresError(sqlErr *SQLError) bool {
	_, stateOk := pgSQLStates[sqlErr.SQLState]
	return sqlErr.Message != "" && stateOk
}
