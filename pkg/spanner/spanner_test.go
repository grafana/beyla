...
func assertMatches(t *testing.T, span *HTTPRequestSpan, method, path, peer string, peerPort, status int, durationMs uint64) {
	assert.Equal(t, method, span.Method)
	assert.Equal(t, path, span.Path)
	assert.Equal(t, method, span.Method)
	assert.Equal(t, peer, span.Peer)
	assert.Equal(t, peerPort, span.PeerPort)
