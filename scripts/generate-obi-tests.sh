...
apply_schema_injections() {
    ...
    # Beyla owns this metric, including its schema after OBI PR #3556 removes it.
    # Drop the inherited definition while generating from older OBI revisions.
    ...
    cat > "$SCHEMAS_DEST/obi/groups/beyla_host_info.yaml" <<'EOF'
groups:
  - id: metric.beyla.traces.host.info
    type: metric
    metric_name: traces.host.info
    instrument: gauge
    unit: ""
    stability: development
    brief: Host identity for correlating telemetry from instrumented applications.
    attributes:
      - id: grafana.host.id
        type: string
        brief: Host identifier used by Grafana to correlate telemetry.
        stability: development
        examples: ["integration-test-host"]
...
EOF
    ...
    # OBI 55e6245d1 removed --include-unreferenced from Weaver live-check.
    # Process definitions previously came from that blanket upstream import;
    # Beyla still emits them in suites using application_process (including
    # owners, via the shared daemonset manifest patched above).
    # Beyla's process exporters are not part of OBI's emitted schema. Declare
    # their six metrics and selected resource attributes so live-check can
    # validate them without importing unrelated process metrics/attributes.
    # Attributes are opt-in because attributes.select can exclude them.
    # Keep this injection across OBI bumps, just like the survey metric below.
    cat > "$SCHEMAS_DEST/obi/groups/x_beyla_process.yaml" <<'EOF'
# Generated by scripts/generate-obi-tests.sh (apply_schema_injections).
# Beyla owns these exporters; OBI does not declare their emitted contract.
# Keep instruments/units aligned with pkg/export/otel/metrics_proc.go.
groups:
  - id: entity.beyla.process
    type: entity
    name: beyla.process
    stability: development
    brief: Process resource attributes emitted by Beyla.
    attributes:
      - ref: process.pid
        role: descriptive
  ...
  - id: metric.beyla.process.cpu.time
    type: metric
    metric_name: process.cpu.time
    instrument: counter
    unit: "s"
    stability: development
    brief: CPU time consumed by the process.
    entity_associations:
      - beyla.process
    attributes:
      - ref: cpu.mode
  ...
  - id: metric.beyla.process.cpu.utilization
    type: metric
    metric_name: process.cpu.utilization
    instrument: gauge
    unit: "1"
    stability: development
    brief: CPU utilization of the process.
    entity_associations:
      - beyla.process
    attributes:
      - ref: cpu.mode
  ...
  - id: metric.beyla.process.memory.usage
    type: metric
    metric_name: process.memory.usage
    instrument: updowncounter
    unit: "By"
    stability: development
    brief: Physical memory used by the process.
    entity_associations:
      - beyla.process

  - id: metric.beyla.process.memory.virtual
    type: metric
    metric_name: process.memory.virtual
    instrument: updowncounter
    unit: "By"
    stability: development
    brief: Virtual memory used by the process.
    entity_associations:
      - beyla.process

  - id: metric.beyla.process.disk.io
    type: metric
    metric_name: process.disk.io
    instrument: counter
    unit: "By"
    stability: development
    brief: Disk bytes transferred by the process.
    entity_associations:
      - beyla.process
    attributes:
  ...
  - id: metric.beyla.process.network.io
    type: metric
    metric_name: process.network.io
    instrument: counter
    unit: "By"
    stability: development
    brief: Network bytes transferred by the process.
    entity_associations:
      - beyla.process
    attributes:
