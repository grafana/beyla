...
package aggregate // import "go.opentelemetry.io/obi/pkg/export/otel/metric/components/aggregate"
...
func (s *histogram[N]) delta(dest *sdkmetricdata.Aggregation) int {
	t := now()
	...
	h, _ := (*dest).(sdkmetricdata.Histogram[N])
	h.Temporality = sdkmetricdata.DeltaTemporality
	...
	for _, val := range s.values {
		...
		if !s.noMinMax {
			hDPts[i].Min = sdkmetricdata.NewExtrema(val.min)
			hDPts[i].Max = sdkmetricdata.NewExtrema(val.max)
		}
...
func (s *histogram[N]) cumulative(dest *sdkmetricdata.Aggregation) int {
	t := now()

	// If *dest is not a metricdata.Histogram, memory reuse is missed. In that
	// case, use the zero-value h and hope for better alignment next cycle.
	h, _ := (*dest).(sdkmetricdata.Histogram[N])
	h.Temporality = sdkmetricdata.CumulativeTemporality
